                ...
                return (
                  <React.Fragment key={p.id}>
                    <tr className="cursor-pointer hover:bg-slate-50/60 transition-colors" onClick={() => setExpanded(isExp ? null : p.id)}>
                      ...
                      <td className="table-td text-right text-xs">{p.sourcePrice != null ? formatCurrency(p.sourcePrice) : '—'}</td>
                      <td className="table-td text-right text-xs font-medium">{p.finalCost != null ? formatCurrency(p.finalCost) : '—'}</td>
                      <td className="table-td text-right text-xs">{p.lowestFbaPrice != null ? formatCurrency(p.lowestFbaPrice) : formatCurrency(p.price)}</td>
                      ...
                      <td className="table-td text-right text-xs text-slate-600">{p.bsr != null ? `#${p.bsr.toLocaleString()}` : '—'}</td>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between text-sm">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-slate-600 px-2">Page {page} / {totalPages}</span>
