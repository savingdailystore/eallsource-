...
export function DeleteRuleButton({ id, asin }: { id: string; asin: string }) {
  ...
  async function handleDelete() {
    setBusy(true);
    await fetch(`/api/repricing/rules/${id}`, { method: 'DELETE' });
    router.refresh();
  }
  ...
  if (confirm) {
    return (
      <span className="flex items-center gap-1">
        ...
        <button onClick={() => setConfirm(false)} className="text-xs text-slate-500 hover:text-slate-300 px-1">
          Cancel
        </button>
      </span>
    );
