  ...
  if (mode === 'disable') {
    return (
      <form onSubmit={confirmDisable} className="space-y-3 max-w-sm">
        <p className="text-sm text-slate-600">Enter your password to turn off two-factor authentication.</p>
  ...
  if (mode === 'setup') {
    return (
      <form onSubmit={confirmEnable} className="space-y-4 max-w-sm">
        ...
        {qr && <img src={qr} alt="MFA QR code" className="w-44 h-44 border border-slate-200 rounded-xl" />}
        <div className="text-xs text-slate-500">
          Can't scan? Enter this key manually:
          <div className="font-mono text-slate-700 break-all mt-1 bg-slate-50 rounded-lg px-2 py-1">{secret}</div>
        </div>
        <div>
