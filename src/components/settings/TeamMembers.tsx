...
const ROLE_BADGE: Record<string, string> = {
  ...
  ANALYST: 'bg-slate-800 text-slate-300',
    ...
    <>
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <h2 className="font-semibold text-slate-50">Team Members</h2>
            ...
            <div key={u.id} className="flex items-center justify-between py-3 gap-3">
              <div className="min-w-0">
                <div className="text-sm font-medium text-slate-50 truncate">
                  {u.name ?? u.email}{isSelf && <span className="text-xs text-slate-500 font-normal"> (you)</span>}
                </div>
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                {editable ? (
                  <select
                    ...
                    className="text-xs border border-slate-800 rounded-lg px-2 py-1 text-slate-200 focus:outline-none focus:border-blue-400"
                  >
                    {ASSIGNABLE.map((r) => <option key={r} value={r}>{r}</option>)}
                  </select>
                ) : (
                  <span className={`badge text-xs ${ROLE_BADGE[u.role] ?? 'bg-slate-800 text-slate-300'}`}>{u.role}</span>
                )}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.7)' }}>
      <div className="bg-slate-900 rounded-2xl shadow-xl w-full max-w-md">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800">
          <h2 className="font-semibold text-slate-50">Invite Team Member</h2>
        ...
        {done ? (
          <div className="px-6 py-6 space-y-4">
            ...
            <div className="bg-slate-800/40 rounded-xl p-4 text-sm space-y-1">
              <div><span className="text-slate-500">Email:</span> <span className="font-mono">{form.email}</span></div>
              <div><span className="text-slate-500">Temp password:</span> <span className="font-mono">{form.password}</span></div>
              <div><span className="text-slate-500">Role:</span> {form.role}</div>
            </div>
          ...
          <form onSubmit={submit} className="px-6 py-5 space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">You'll share this with the user. They can change it after signing in.</p>
            </div>
